...
def load_pretrained_params(model, pretrained_model, logger):
    state_dict_1 = torch.load(pretrained_model, map_location="cpu")
    state_dict_1 = state_dict_1['state_dict']
    # state_dict_2 = torch.load("/home/imuse/mazhibin/AdvancedLiterateMachinery/OCR/OPT-125M/pytorch_model.bin", map_location="cpu")
    # print(state_dict_1.keys())
    # print(state_dict_2.keys())
    # exit()

    checkpoint = {}
    checkpoint.update(state_dict_1)
    # checkpoint.update(state_dict_2)
    # checkpoint = torch.load(pretrained_model, map_location=torch.device("cpu"))
    original_state_dict = checkpoint
    # print(original_state_dict.keys())

    # 构建新的 state_dict，只保留 visual 开头的参数并重命名为 encoder
    ...
    for name, param in original_state_dict.items():
        if name.startswith("visual."):
            new_name = name.replace("visual.", "encoder.", 1) 
