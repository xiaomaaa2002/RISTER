...
class RecTVResize(object):
    ...
    def __call__(self, data):
        ...
        angle = 0
        img = T.functional.rotate(img, angle, expand=True)
        resized_image = F.resize(img, (imgH, resized_w),
                                 interpolation=self.interpolation)
        # resized_image = T.functional.rotate(resized_image, angle, expand=True)
...
class ScalePreserveResize(object):

    def __init__(self, image_shape=[128, 128], padding=True, **kwargs):
        self.padding = padding
        self.image_shape = image_shape
        self.interpolation = T.InterpolationMode.BICUBIC
        transforms = []
    ...
    def __call__(self, data):
        ...
        # 对角线归一化
        S = int(round(math.sqrt(w**2 + h**2)))
        factor = imgH / S
        resH, resW = round(factor * h), round(factor * w)

        img = F.resize(img, (resH, resW), interpolation=self.interpolation)

        # === 同步构造 mask（PIL 阶段） ===
        mask = Image.new("L", (resW, resH), 255)  # 有效区域 = 255

        # canvas
        canvas = Image.new("RGB", (imgH, imgW), (0, 0, 0))
        mask_canvas = Image.new("L", (imgH, imgW), 0)
        ...
        # 旋转（如果以后 angle ≠ 0 也自动支持）
        angle = 0
        canvas = T.functional.rotate(canvas, angle)
        mask_canvas = T.functional.rotate(mask_canvas, angle)

        # 图像 transforms
        img = self.transforms(canvas)

        # mask → Tensor（不做 normalize）
        mask = torch.from_numpy(
            np.array(mask_canvas, dtype=np.float32)
        ) / 255.0
...
class ScalePreserveResizeTest(object):
...
class RobustScannerRecResizeImgTest(object):
