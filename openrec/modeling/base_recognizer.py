...
class BaseRecognizer(nn.Module):
    ...
    def __init__(self, config):
        ...
        if 'PolarTransform' not in config or config['PolarTransform'] is None:
            self.use_polartransform = False
        else:
            self.use_polartransform = True
            config['PolarTransform']['in_channels'] = in_channels
            self.polartransform = build_transform(config['PolarTransform'])
            in_channels = self.polartransform.out_channels
    ...
    def forward(self, x, data=None):
        if self.use_transform:
            x = self.transform(x)
        if self.use_polartransform:
            if len(data) == 3:
                x = self.polartransform(x, data[2])
            else:
                x = self.polartransform(x, mask=None)
        if self.use_encoder:
            x = self.encoder(x)
        if self.use_decoder:
