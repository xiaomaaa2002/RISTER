...
class WrapConv(nn.Module):
    ...
    def forward(self, x):
        out = self.conv(torch.concat([x[:,:,-1,:].unsqueeze(2), x, x[:,:,0,:].unsqueeze(2)], dim=2))
        out = self.bn(out)
        out = self.relu(out)
        return out
...
class MORN(nn.Module):
    ...
    def __init__(self, in_channels, target_shape=[32, 128], enhance=1):
    ...
    def forward(self, x):
        ...
        grid_rec = grid_y + p * offsets_grid
        grid_rec[grid_rec < -1] += 2
        grid_rec[grid_rec > 1] -= 2
        offsets_x = torch.cat([grid_x, grid_rec], 3)
