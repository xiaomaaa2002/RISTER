...
class RITextDecoder(nn.Module):
    ...
    def __init__(
        ...
        super(RITextDecoder, self).__init__()
    ...
    def forward_test(self, src):
        ...
        for len_dec_seq in range(0, self.max_len):
            # if len_dec_seq == 1:
            #     exit()
            dec_seq_embed = self.embedding(dec_seq[:, :len_dec_seq + 1])  # N dim 26+10 # </s>  012 a
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query, key=None, attn_mask=None):
        ...
        #     for i in range(32):
        #         print(attn.shape)
        #         pic = toPIL(attn[i][0][-1].reshape(4, 32)*5)
        #         pic.save('img/oripic'+str(i)+'map.jpg')
        #     exit()
